...
class Base():
    ...
    def find(self, locator):
        ...
        if not isinstance(locator, tuple):
            raise LocatorTypeError("参数类型错误，locator必须是元祖类型：locator = ('id','value1')")
        else:
            ...
            try:
                ...
                raise ElementNotFound("定位元素出现超时，未找到元素！！！")
            return ele
    ...
    def is_alert(self, timeout=3):
        """弹框"""
        try:
            result = WebDriverWait(self.driver, timeout, self.t).until(
                EC.alert_is_present())
            return result
        except:
            return False
